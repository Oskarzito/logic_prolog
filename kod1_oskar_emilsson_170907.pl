...
son(X, Y):- (mother(Y, X); father(Y, X)), man(X).

daughter(X, Y):- (mother(Y, X); father(Y, X)), woman(X).

child(X, Y):- mother(Y, X); father(Y, X).

married(X, Y):- (husband(X, Y), wife(Y, X));(husband(Y, X), wife(X, Y)).

parent(X, Y):- daughter(Y, X); son(Y, X).

grandfather(X, Y):- parent(X, Z), parent(Z, Y), man(X).

grandmother(X, Y):- parent(X, Z), parent(Z, Y), woman(X).

brother(X, Y):- (parent(M, X), parent(M, Y)), (parent(F, X), parent(F, Y)), man(X), man(M), woman(F), X \= Y.

sister(X, Y):- (parent(M, X), parent(M, Y)), (parent(F, X), parent(F, Y)), woman(X), man(M), woman(F), X \= Y.

sibling(X, Y):- (parent(M, X), parent(M, Y)), (parent(F, X), parent(F, Y)), man(M), woman(F), X \= Y.


brother_in_law(X, Y):- ((married(X, SPOUSE), sibling(Y, SPOUSE)); (sibling(X, SPOUSE), married(SPOUSE, Y));
						(married(X, SPOUSE), sibling(SPOUSE, SIBTOSPOUSE), married(SIBTOSPOUSE, Y))), man(X).

sister_in_law(X, Y):- ((married(X, SPOUSE), sibling(Y, SPOUSE)); (sibling(X, SPOUSE), married(SPOUSE, Y));
						(married(X, SPOUSE), sibling(SPOUSE, SIBTOSPOUSE), married(SIBTOSPOUSE, Y))), woman(X).

sibling_in_law(X, Y):- (married(X, SPOUSE), sibling(Y, SPOUSE)); (sibling(X, SPOUSE), married(SPOUSE, Y));
						(married(X, SPOUSE), sibling(SPOUSE, SIBTOSPOUSE), married(SIBTOSPOUSE, Y)).

father_in_law(X, Y):- father(X, SPOUSE), married(SPOUSE, Y).

mother_in_law(X, Y):- mother(X, SPOUSE), married(SPOUSE, Y).

uncle(X, Y):- (brother(X, PARENT), parent(PARENT, Y)); 
				(husband(X, PARENTSISTER), sister(PARENTSISTER, PARENT), parent(PARENT, Y)).


aunt(X, Y):- (sister(X, PARENT), parent(PARENT, Y));
				(wife(X, PARENTBROTHER), brother(PARENTBROTHER, PARENT), parent(PARENT, Y)).


cousin(X, Y):- child(X, PARENT), sibling(PARENT, PARENTSIBLING), parent(PARENTSIBLING, Y).

nephew(X, Y):- ((son(X, PARENT), sibling(PARENT, Y)); (son(X, PARENTINLAW), sibling_in_law(PARENTINLAW, Y))), man(X).

niece(X, Y):- ((daughter(X, PARENT), sibling(PARENT, Y)); (daughter(X, PARENTINLAW), sibling_in_law(PARENTINLAW, Y))), woman(X).
